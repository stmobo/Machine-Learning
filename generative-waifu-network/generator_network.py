...
def generator_parameters(parser):
    ...
    parser.add_argument('--gen-layer-depth', type=int, default=64, help='Final (base) number of generator deconv filters (before image output layer)')
...
class GeneratorNet:
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for layer in range(self.args.gen_layers):
                if self.args.deception_modules:
                    net = self.deception_module(net, scope='Deception{:d}'.format(layer))
                else:
                    net = self.standard_deconv_layer(net, scope='ConvT{:d}'.format(layer))

            return self.output_layer(net)

    def activation_fn(self):
        if self.args.generator_activation == 'relu':
            return tf.nn.relu
        elif self.args.generator_activation == 'lrelu':
            return leaky_relu
        elif self.args.generator_activation == 'selu':
            return selu.selu
        raise ValueError("Invalid value for --generator-activation: " + self.args.generator_activation)

    def initializer_fn(self):
        if self.args.generator_activation == 'relu' or self.args.generator_activation == 'lrelu':
            return None
        else:
    ...
    def input_layer(self, noise_in, labels_in, scope='Input'):
        ...
        input_layer_height, input_layer_width = self.args.output_height // input_layer_divisor, self.args.output_width // input_layer_divisor

        batch_size = noise_in.shape.as_list()[0]
        projection_units = input_layer_height * input_layer_width

        with tf.variable_scope(scope):
            with slim.arg_scope([slim.fully_connected, slim.conv2d, slim.conv2d_transpose], activation_fn=self.activation_fn(), initializer_fn=self.initializer_fn())
                ...
                net = tf.conv2d(input_layer,
                    self.args.gen_layer_depth // 2,
                    kernel_size=1,
                    scope='input-conv'
                )
    ...
    def deception_module(self, tensor_in, scope='Deception'):
        ...
        output_depth = self.args.gen_layer_depth
        bottleneck_depth = self.args.gen_bottleneck_depth

        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=self.activation_fn(), weights_initializer=self.initializer_fn()):
    ...
    def standard_deconv_layer(self, tensor_in, scope='ConvT'):
        batch_size, layer_height, layer_width, layer_depth = tensor_in.shape.as_list()
        output_height, output_width = layer_height * 2, layer_width * 2
        output_depth = self.args.gen_layer_depth
